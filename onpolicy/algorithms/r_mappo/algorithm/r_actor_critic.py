...
class R_Actor(nn.Module):
    ...
    def __init__(self, args, obs_space, action_space, device=torch.device("cpu")):
        ...
        self._use_smac_hetero = (
            self._use_hetero_graph and getattr(args, 'env_name', None) in ['StarCraft2', 'SMAC', 'SMACv2', 'StarCraft2v2']
        )
        ...
        if self._use_hetero_graph:
            ...
            if self._use_world_comm_hetero:
                ...
                if self.agent_state_dim != 8 or self.landmark_dim != 4 or self.neighbor_dim != 5:
                    raise ValueError(
                        "simple_world_comm HeteroGraph requires "
                        "--agent_state_dim 8 --landmark_dim 4 --neighbor_dim 5"
                    )
    ...
    def _split_world_comm_hetero_obs(self, obs):
        ...
        if is_adversary_policy:
            ...
            idx += G * 2
            in_forest = self._pad_last_dim(obs[:, idx:idx + F], 2)
            idx += F
            prey_forest = self._pad_last_dim(obs[:, idx:idx + G], 2)

            agent_state = torch.cat([self_phys, in_forest, prey_forest], dim=-1)
    ...
    def _extract_features(self, obs):
        ...
        if self._use_hetero_graph:
            # Detect environment type based on scenario_name or use_smac_hetero flag
            use_smac_hetero = getattr(self, '_use_smac_hetero', False)
