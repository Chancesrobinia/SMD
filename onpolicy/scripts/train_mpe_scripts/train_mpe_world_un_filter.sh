#!/bin/sh
# =============================================================================
# Train serial HeteroGraph on MPE simple_world_comm without Hop-3 filtering.
#
# This keeps the separated-policy adversary-only path required by
# simple_world_comm. Adversary actors use the serial hetero graph backbone;
...
dim_c=4
algo="rmappo"
exp="hetero_graph_serial_unfilter_world_comm"
seed_max=1
...
for seed in $(seq "${seed_max}"); do
    ...
    CUDA_VISIBLE_DEVICES=0 python train/train_mpe.py \
        ...
        --use_valuenorm \
        --use_wandb \
        --wandb_name "hetero_graph_serial_unfilter_world_comm" \
        --user_name "yuchao" \
        --share_policy \
        --use_hetero_graph \
