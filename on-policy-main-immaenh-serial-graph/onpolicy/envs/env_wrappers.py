...
def stack_obs(obs):
    """Stack vectorized-env observations while allowing heterogeneous agent dims."""
    try:
        return np.stack(obs)
    except ValueError:
        if len(obs) > 0 and hasattr(obs[0], "__len__"):
            obs0 = list(obs[0])
            arr = np.empty((len(obs), len(obs0)), dtype=object)
            for env_id, env_obs in enumerate(obs):
                env_obs = list(env_obs)
                if len(env_obs) != len(obs0):
                    raise ValueError("All vectorized envs must return the same number of agent observations.")
                arr[env_id, :] = env_obs
            return arr
        arr = np.empty(len(obs), dtype=object)
        arr[:] = obs
        return arr
...
class GuardSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        return stack_obs(obs), np.stack(rews), np.stack(dones), infos
...
class DummyVecEnv(ShareVecEnv):
    ...
    @staticmethod
    def _array_or_object(items):
        return stack_obs(items)
    ...
    def reset(self):
        ...
        return self._array_or_object(obs)
